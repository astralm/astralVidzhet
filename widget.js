...
var AstralWidget = function(){
	...
	this.createElement = function(tag, classList, options, html){
		...
		if(options){
			for(var key in options){
				try {
					var key = key.match(/(data)(.*)/).filter(function(item, key){return key > 0}).join("-");
				} catch (e){

				}
				dom.setAttribute(key, options[key] || options[key.split("-").join("")]);
			}
		}
		if(html){
	...
	this.events = {
		intervalMove: (function(){
	      if(this.move){
	          ...
	          if(offsetLeft >= 0 && offsetLeft + width <= screenWidth){
	            ...
	            this.left = +widget.style.left.match(/[0-9]*/)[0];
	          }
	        }
	        if(this.moveY){
	          ...
	          if(offsetTop >= 0 && offsetTop + height <= screenHeight){
	            ...
	            this.top = +widget.style.top.match(/[0-9]*/)[0];
	          }
	        }
	      }
	    }).bind(this),
		...
		intervalResize: (function(){
			if(this.resize){
				var widget = this.dom.widgetWrapper;
				if(this.resizeRight || this.resizeTopRight || this.resizeBottomRight){
					var screenWidth = window.innerWidth,
						width = widget.offsetWidth,
						oLeft = widget.offsetLeft,
						diffX = this.diffX,
						left = this.left;
					widget.style.width = (diffX - (left + width)) + width + "px";
					width = widget.offsetWidth;
					if(oLeft + width >= screenWidth){
						widget.style.width = screenWidth - oLeft + "px";
					}
				}
				if(this.resizeLeft || this.resizeTopLeft || this.resizeBottomLeft){
					var width = this.width,
						oWidth = widget.offsetWidth,
						diffX = this.diffX,
						left = this.left;
					widget.style.width = width + (left - diffX) + "px";
					if(width + (left - diffX) >= 300){
						widget.style.left = left - (left - diffX) + "px";
					}
				}
				if(this.resizeTop || this.resizeTopLeft || this.resizeTopRight){
					var height = this.height,
						diffY = this.diffY,
						top = this.top,
						oHeight = widget.offsetHeight;
					if(oHeight != 272){
						widget.style.top = top - (top - diffY) + "px";
					}
					if(top - (top - diffY) > 0){
						widget.style.height = height + (top - diffY) + "px";
						this.dom.widgetContainer.style.height = height + (top - diffY) - 72 + "px";
					} else {
						widget.style.top = 0 + "px";
					}
				}
				if(this.resizeBottom || this.resizeBottomRight || this.resizeBottomLeft){
					var diffY = this.diffY,
						top = this.top,
						height = this.height,
						screenHeight = window.innerHeight;
					widget.style.height = diffY - top + "px";
					this.dom.widgetContainer.style.height = diffY - top - 72 + "px";
					var oTop = widget.offsetTop,
						oHeight = widget.offsetHeight;
					if(oTop + oHeight > screenHeight){
						widget.style.height = screenHeight - oTop + "px";
						this.dom.widgetContainer.style.height = screenHeight - oTop - 72 + "px";
					}
				}
			}
		}).bind(this),
		resizeStart: (function(e){
			var widget = this.dom.widgetWrapper;
			this.left = widget.offsetLeft;
			this.width = widget.offsetWidth;
			this.height = widget.offsetHeight;
			this.top = widget.offsetTop;
			this.resize = true;
		}).bind(this),
		resizeEnd: (function(){
			this.resize = false;
		...
		resizeRight: (function(e){
			this.changeResize("Right");
			if(!e.touches){
				e.preventDefault();
			}
		}).bind(this),
		resizeLeft: (function(e){
			this.changeResize("Left");
			if(!e.touches){
				e.preventDefault();
			}
		}).bind(this),
		resizeTop: (function(e){
			this.changeResize("Top");
			if(!e.touches){
				e.preventDefault();
			}
		}).bind(this),
		resizeBottom: (function(e){
			this.changeResize("Bottom");
			if(!e.touches){
				e.preventDefault();
			}
		}).bind(this),
		resizeTopRight: (function(e){
			this.changeResize("TopRight");
			if(!e.touches){
				e.preventDefault();
			}
		}).bind(this),
		resizeTopLeft: (function(e){
			this.changeResize("TopLeft");
			if(!e.touches){
				e.preventDefault();
			}
		}).bind(this),
		resizeBottomRight: (function(e){
			this.changeResize("BottomRight");
			if(!e.touches){
				e.preventDefault();
			}
		}).bind(this),
		resizeBottomLeft: (function(e){
			this.changeResize("BottomLeft");
			if(!e.touches){
				e.preventDefault();
			}
		}).bind(this),
			...
			if(dialog){
	            ...
	            dialog.map(function(item){
	            	if(item.question_message){
		            	this.dom.widgetContainer.appendChild(this.createElement("table", ["widget_message", "widget_table"], null, [
		            		this.createElement("thead", null, null, [
		            			this.createElement("tr", null, null, [
		            				this.createElement("td", null, null, [
		            					this.createElement("h4", ["widget_title"], null, [document.createTextNode("Вы")])
		            				]),
		            				this.createElement("td", null, null, [
		            					this.createElement("div", ["widget_date"], null, [document.createTextNode(item.question_date_formated)])
		            				])
		            			])
		            		]),
		            		this.createElement("tbody", null, null, [
		            			this.createElement("tr", null, null, [
		            				this.createElement("td", ["widget_message_body_column"], {colspan: "2"}, [
		            					this.createElement("p", ["widget_text"], null, [document.createTextNode(item.question_message)])
		            				])
		            			])
		            		])
		            	]));
	            	...
	            	if(item.answer_message){
	            		this.dom.widgetContainer.appendChild(this.createElement("table", ["widget_message", "widget_table"], null, [
		            		this.createElement("thead", null, null, [
		            			this.createElement("tr", null, null, [
		            				this.createElement("td", null, null, [
		            					this.createElement("h4", ["widget_title"], null, [document.createTextNode("Консультант")])
		            				]),
		            				this.createElement("td", null, null, [
		            					this.createElement("div", ["widget_date"], null, [document.createTextNode(item.answer_date_formated)])
		            				])
		            			])
		            		]),
		            		this.createElement("tbody", null, null, [
		            			this.createElement("tr", null, null, [
		            				this.createElement("td", ["widget_message_body_column"], {colspan: "2"}, [
		            					this.createElement("p", ["widget_text"], null, [document.createTextNode(item.answer_message)])
		            				])
		            			])
		            		])
		            	]));
	            ...
	            this.dom.widgetContainer.scrollTop = this.dom.widgetContainer.scrollHeight;
            }
		},
	...
	this.initDom = function(){
		this.dom = {};
		this.dom.tree = this.createElement("noindex", null, null, [
			this.createElement("table", ["widget_wrapper", "widget_table", "widget_close"], {id: "widgetWrapper"}, [
				this.createElement("thead", ["widget_header"], {id: "widgetHeader"}, [
					this.createElement("tr", null, null, [
						this.createElement("td", null, null, [
							this.createElement("h3", ["widget_title"], null, [document.createTextNode("Астрал")])
						]),
						this.createElement("td", null, null, [
							this.createElement("div", ["widget_button", "right"], {id: "widgetCloseButton"}, [
								this.createElement("i", ["material-icons"], null, [document.createTextNode("")])
							])
						])
					])
				]),
				this.createElement("tbody", ["widget_body"], null, [
					this.createElement("tr", null, null, [
						this.createElement("td", null, {colspan: "2"}, [
							this.createElement("table", ["widget_table"], null, [
								this.createElement("tr", null, null, [
									this.createElement("td", ["widget_body_content_column"], null, [
										this.createElement("div", ["widget_body_height_container"], {id: "widgetContainer"})
									])
								])
							])
						])
				...
				this.createElement("tfoot", ["widget_footer"], null, [
					this.createElement("tr", null, null, [
						this.createElement("td", null, {colspan: "2"}, [
							this.createElement("table", ["widget_table"], null, [
								this.createElement("tr", null, null, [
									this.createElement("td", null, null, [
										this.createElement("div", ["widget_input"], {contenteditable: "true", dataph: "Введите сообщение", id: "widgetInput"})
									]),
									this.createElement("td", null, null, [
										this.createElement("div", ["widget_button"], {id: "widgetButton"}, [
											this.createElement("i", ["material-icons"], null, [document.createTextNode("")])
										])
									])
								])
				...
				this.createElement("div", ["widget_shadow", "widget_shadow_vertical", "widget_shadow_vertical_right"], {id: "widgetShadowVerticalRight"}),
				this.createElement("div", ["widget_shadow", "widget_shadow_vertical", "widget_shadow_vertical_left"], {id: "widgetShadowVerticalLeft"}),
				this.createElement("div", ["widget_shadow", "widget_shadow_horizontal", "widget_shadow_horizontal_top"], {id: "widgetShadowHorizontalTop"}),
				this.createElement("div", ["widget_shadow", "widget_shadow_horizontal", "widget_shadow_horizontal_bottom"], {id: "widgetShadowHorizontalBottom"}),
				this.createElement("div", ["widget_shadow", "widget_shadow_angle", "widget_shadow_angle_right_top"], {id: "widgetShadowAngleRightTop"}),
				this.createElement("div", ["widget_shadow", "widget_shadow_angle", "widget_shadow_angle_right_bottom"], {id: "widgetShadowAngleRightBottom"}),
				this.createElement("div", ["widget_shadow", "widget_shadow_angle", "widget_shadow_angle_left_bottom"], {id: "widgetShadowAngleLeftBottom"}),
				this.createElement("div", ["widget_shadow", "widget_shadow_angle", "widget_shadow_angle_left_top"], {id: "widgetShadowAngleLeftTop"})
			]),
			this.createElement("table", ["widget_close_table", "widget_table"], null, [
				this.createElement("tbody", null, null, [
					this.createElement("tr", null, null, [
						this.createElement("td", null, null, [
							this.createElement("div", ["widget_button"], {id: "widgetOpenButton"}, [
								this.createElement("i", ["material-icons"], null, [document.createTextNode("")])
							])
						])
					])
		...
		this.dom.tree.querySelectorAll("*[id]").forEach((function(item){
			this.dom[item.getAttribute('id')] = item;
			item.removeAttribute('id');
		}).bind(this));
	};
	this.bindEvents = function(object, events){
		for (var i = 0; i < events.length; i++){
			var event = events[i];
			object.addEventListener(event[0], event[1]);
		}
	};
	this.changeResize = function(side){
		this.resizeRight = false;
		this.resizeTop = false;
		this.resizeBottom = false;
		this.resizeLeft = false;
		this.resizeBottomRight = false;
		this.resizeBottomLeft = false;
		this.resizeTopRight = false;
		this.resizeTopLeft = false;
		if(side){
			this["resize" + side] = true;
		}
	};
	this.initEvents = function(){
		setInterval(this.events.intervalMove, 1);
		setInterval(this.events.intervalResize, 1);
		...
		this.bindEvents(this.dom.widgetHeader, [
			["mousedown", this.events.start],
			["touchstart", this.events.start]
		]);
		this.bindEvents(window, [
			["mouseup", this.events.end],
			["touchend", this.events.end],
			["touchmove", this.events.move],
			['mousemove', this.events.move],
			["mouseup", this.events.resizeEnd],
			["touchend", this.events.resizeEnd],
			["touchmove", this.events.resize],
			['mousemove', this.events.resize]
		]);
		this.bindEvents(this.dom.widgetShadowVerticalRight, [
			['mousedown', this.events.resizeStart],
			['touchstart', this.events.resizeStart],
			['mousedown', this.events.resizeRight],
			['touchstart', this.events.resizeRight]
		]);
		this.bindEvents(this.dom.widgetShadowVerticalLeft, [
			['mousedown', this.events.resizeStart],
			['touchstart', this.events.resizeStart],
			['mousedown', this.events.resizeLeft],
			['touchstart', this.events.resizeLeft]
		]);
		this.bindEvents(this.dom.widgetShadowHorizontalBottom, [
			['mousedown', this.events.resizeStart],
			['touchstart', this.events.resizeStart],
			['mousedown', this.events.resizeBottom],
			['touchstart', this.events.resizeBottom]
		]);
		this.bindEvents(this.dom.widgetShadowHorizontalTop, [
			['mousedown', this.events.resizeStart],
			['touchstart', this.events.resizeStart],
			['mousedown', this.events.resizeTop],
			['touchstart', this.events.resizeTop]
		]);
		this.bindEvents(this.dom.widgetShadowAngleLeftTop, [
			['mousedown', this.events.resizeStart],
			['touchstart', this.events.resizeStart],
			['mousedown', this.events.resizeTopLeft],
			['touchstart', this.events.resizeTopLeft]
		]);
		this.bindEvents(this.dom.widgetShadowAngleLeftBottom, [
			['mousedown', this.events.resizeStart],
			['touchstart', this.events.resizeStart],
			['mousedown', this.events.resizeBottomLeft],
			['touchstart', this.events.resizeBottomLeft]
		]);
		this.bindEvents(this.dom.widgetShadowAngleRightBottom, [
			['mousedown', this.events.resizeStart],
			['touchstart', this.events.resizeStart],
			['mousedown', this.events.resizeBottomRight],
			['touchstart', this.events.resizeBottomRight]
		]);
		this.bindEvents(this.dom.widgetShadowAngleRightTop, [
			['mousedown', this.events.resizeStart],
			['touchstart', this.events.resizeStart],
			['mousedown', this.events.resizeTopRight],
			['touchstart', this.events.resizeTopRight]
		]);
	};
